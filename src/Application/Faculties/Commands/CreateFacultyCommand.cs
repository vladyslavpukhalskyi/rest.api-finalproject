    ...
    IFacultyRepository facultyRepository) : IRequestHandler<CreateFacultyCommand, Result<Faculty, FacultyException>>
{
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            var technicalEquipment = new TechnicalEquipment
            {
                Name = "Memory",
                Weight = 33.3,
                Parts = [new TechnicalEquipmentPart {Name = "Rate"}]
            };

            var entity = Faculty.New(FacultyId.New(), name, FacultyEquipment.NewTechnicalEquipment(technicalEquipment));
